...
fn new_float(c: &mut Criterion) {
    ...
    group
        .bench_function(BenchmarkId::from_parameter("0.0"), |b| {
            b.iter(|| RawBox::from_f64(black_box(0.0)))
        })
        .bench_function(BenchmarkId::from_parameter("1.0"), |b| {
            b.iter(|| RawBox::from_f64(black_box(1.0)))
        })
        .bench_function(BenchmarkId::from_parameter("inf"), |b| {
            b.iter(|| RawBox::from_f64(black_box(f64::INFINITY)))
        })
        .bench_function(BenchmarkId::from_parameter("-inf"), |b| {
            b.iter(|| RawBox::from_f64(black_box(f64::NEG_INFINITY)))
        })
        .bench_function(BenchmarkId::from_parameter("NaN"), |b| {
            b.iter(|| RawBox::from_f64(black_box(f64::NAN)))
        });
}
...
fn is_f64(c: &mut Criterion) {
    ...
    let d0 = RawBox::from_data(RawTag::new(false, 1), [0; 6]).unwrap();
    let dfull = RawBox::from_data(RawTag::new(true, 7), [255; 6]).unwrap();
...
fn into_f64(c: &mut Criterion) {
    ...
    let f0 = RawBox::from_f64(0.0);
    let fnan = RawBox::from_f64(f64::NAN);
    let tagf1 = RawBox::from_data(RawTag::new(false, 1), [0; 6]).unwrap();
    let tagt7 = RawBox::from_data(RawTag::new(true, 7), [255; 6]).unwrap();
